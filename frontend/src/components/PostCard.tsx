...
const PostCard = (props: Props) => {
    ...
    const repost = async () => {
        try {
            const response = await fetch(`http://localhost:3000/api/posts/${post.id}/repost`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`
    ...
    const reply = async () => {
        try {
            const response = await fetch(`http://localhost:3000/api/posts/${post.id}/reply`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`
        ...
        if (!isReplyOpen) {
            try {
                const response = await fetch(
                    `http://localhost:3000/api/posts/${post.id}/replies`,
                    {
                        headers: {
                            "Authorization": `Bearer ${token}`
                        }
